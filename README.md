...
- ``telegram-destination-id`` This is the __numeric__ Telegram user ID. This is the person that will receive our program's message(s). You can use the Telegram bot ``userinfobot`` for the retrieval of your numeric Telegram user ID. This value has to be specified if the program is instructed to send data to Telegram.
